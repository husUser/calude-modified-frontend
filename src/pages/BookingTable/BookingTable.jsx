...
function BookingTable() {
  ...
  // The active two-week period, as computed by the backend (the single
  // source of truth). The frontend no longer derives this itself.
  ...
  // Fetch the currently active two-week booking period from the backend
  // (GET /booking/activePeriod). This is recomputed by the backend from
  // the current instant every time it's called, so refreshing the page
  // never changes which period is "current" mid-window, and the exact
  // same rule the backend enforces on booking creation is what's shown here.
  const fetchActivePeriod = async () => {
    try {
      const response = await axiosInstance.get(`/booking/activePeriod`);
      setPeriodInfo(response?.data || null);
    } catch (error) {
      console.error("Error fetching active booking period:", error);
      setPeriodInfo(null);
    }
  };

  // Build the 14 displayed dates directly from the backend's ISO date
  // list ('YYYY-MM-DD'), rather than computing "this week's Monday" from
  // the browser's own clock/timezone.
  const daysOfWeek = useMemo(() => {
    if (!periodInfo?.dates) return [];
    return periodInfo.dates.map((iso) => {
  ...
  // IST is UTC+5:30 with no DST — mirrors utils/bookingPeriod.js on the
  // backend, so "today"/"past slot" checks agree with the server
  // regardless of the visiting browser's own local timezone.
  ...
  const isPastTimeSlot = (timeRange, dayDate) => {
    ...
    const todayStart = new Date(nowIST.year, nowIST.month, nowIST.date);
    todayStart.setHours(0, 0, 0, 0);
    if (dayDate < todayStart) return true;

    // if same day, compare end time vs now (IST)
